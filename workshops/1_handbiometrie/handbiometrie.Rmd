...
Biometrie betekent het vaststellen van meetbare eigenschappen van levende wezens. Dit kan bijvoorbeeld betekenen levensduur, sexe, lichaamsgewicht en lengte. Biometrische kenmerken worden gebruikt in je paspoort.

Vandaag dag ga je met een groepje van 3-4 medestudenten bezig met het vaststellen van de hand lengte / breedte ratio, in het bijzonder in afhankelijkheid van sexe.

Aan de hand van dit onderwerp gaan we in heel eenvoudige vorm de ***wetenschappelijke cyclus*** doorlopen. 

:::info
Voor dit eerste experiment in de module zal je erg erg geleid werken. De data verzameling doe je echt zelf maar de dataverwerking en -analyse grotendeels naar voorbeeld. In de volgende experimenten van deze module ga je steeds meer onderdelen van dit proces zelf bedenken en uitvoeren, totdat je in de laatste fase geheel zelfstandig aan het werk gaat.
:::

### Benodigde voorkennis

:::attention
Voordat je dit onderdeel kan doen moet je het volgende beheersen:  

- Werken met RStudio
- Basiskennis hebben van de programmeertaal R
- Basiskennis hebben van RMarkdown.
:::

De docent zal je een aantal bronnen en/of lessen aanbieden om je op weg te helpen.
...
In de volgende module ga je hier veel dieper op in, als je daadwerkelijk statistische toetsen gaat uitvoeren.

### Ontwerp het experiment

Als je je onderzoeksvraag hebt geformuleert zal je een manier moeten bedenken om die vraag beantwoord te krijgen. Als we de onderzoeksvraag wat gedetailleerder bekijken zien we daar drie verschillende aspecten (variabelen) in zitten: de sexe, de hand lengte en de hand breedte. We zullen dus een manier moeten vinden om van een groep mensen deze drie variabelen vast te stellen. Hoe groot deze groep moet zijn om een betrouwbaar antwoord te vinden is een heel belangrijke vraag die je bij een volgende module zeker gaat behandelen. Voor nu kiezen we gewoon een klein aantal waarvan het haalbaar is om die in ongeveer een uur tijd te meten.

Sommige wetenschappers zullen overigens tegenwerpen dat je pas van een experiment spreekt als ar sprake is van ingrijpen, en dat is hier natuurlijk niet het geval. Wij hanteren hier de Wikipedia definitie van een experiment:
...
*Van `r number_of_subjects` proefpersonen, gelijk verdeeld over mannen en vrouwen en willekeurig geselecteerd in en rond het schoolgebouw, zal de sexe gevraagd worden en vervolgens de hand lengte en breedte gemeten worden.*

### Ontwerp het protocol

Om te garanderen dat verschillende onderzoekers de metingen op dezelfde ***reproduceerbare*** manier doen wordt de uitvoering van een experiment vaak in een ***protocol*** vastgelegd. 

:::definition
<h4>Protocol</h4>
Een (wetenschappelijk) protocol beschrijft de volgorde en uitvoering van stappen die in een experiment gezet moeten worden om tot een reproduceerbaar resultaat te komen.
:::
...
2. Online spreadsheet om resultaten vast te leggen, met toegang hiertoe op locatie.  
...
Dit type databestand noemen we ***csv*** wat staat voor "comma-separated values". Het is dan ook het handigste als je je bestand een `.csv` extensie geeft.
...
:::info
<h4>Wat je moet inleveren </h4>
Voor de verwerking en analyse van je eigen data maak je een nieuw RMarkdown document aan. Daarin doorloop je dezelfde onderzoeksstappen als hieronder, maar dan met je eigen data. Aan het einde van de dag gaat dat document, in originele en "geknitte" vorm, samen met je databestand, in je portfolio. Let op dat je _alle_ stappen hebt doorlopen voordat je inlevert. Gebruik ook de RStudio spellingscontrole!  
Vergeet niet jullie namen en studentnummers bovenaan te zetten!
:::
...
technician_ID <- rep(c(3134, 3245), each = 20)
...
Voor je eigen data zal je moeten uitzoeken hoe je deze moet inlezen van bestand naar een `data.frame` in geheugen.

### Beschrijvende statistiek

Je hebt je data verzameld en ingelezen. Nu is het tijd om een eerste verkenning ervan uit te voeren.
...
De R code in de _chunks_ hieronder zijn waarschijnlijk iets gevorderder dan wat je tot nu toe geleerd hebt. Je hoeft dit nog niet zelf te kunnen doen, maar je moet wel snappen wat er gebeurd.
...
mean(sim_data[sim_data$gender == "f" , "hand_length"])
mean(sim_data[sim_data$gender == "m" , "hand_length"])
```

maar efficienter en mooier is het om dit "automatisch" te doen:

```{r means-split-over-gender-semi-manual}
tapply(X = sim_data$hand_length, 
       INDEX = sim_data$gender, 
       FUN = mean)
```
...
Kan jij bovenstaande aanpassen zodat ook de standaarddeviatie (sd) wordt gerapporteerd? Is er verschil in spreiding van de data tussen de sexen?
...
:::definition
<h4>Boxplot</h4>
Dit is een grafische weergave van de vijf-getallensamenvatting. Deze bestaat uit het minimum, het eerste kwartiel, de mediaan (of tweede kwartiel), het derde kwartiel en het maximum van de waargenomen data. Uitbijters worden als losse punten weergegeven. 
:::
...
```{r boxplot-split-over-gender, fig.asp=.75, out.width='80%', fig.align='center'}
par(mfrow = c(1, 3))

my_plot <- function(col_name) {
    f = paste0(col_name, " ~ gender")
    boxplot(formula = formula(f),
        data = sim_data,
        xlab = "sexe",
        ylab = col_name,
...
for(col_name in c("hand_width", "hand_length", "ratio")) {
    my_plot(col_name)
}
```
...
:::info
<h4>De stripchart functie</h4>
Gebruik de <code>stripchart()</code> functie als je ook de datapunten zelf wil laten zien
:::
...
```{r ratio-histogram, fig.asp=1.3, out.width='70%', fig.align='center', results='hold'}
par(mfrow = c(3, 1))
hist(sim_data[sim_data$gender == "f", "ratio"],
     col = "darkred", 
     breaks = 10,
     xlab = "length/width ratio",
     xlim = range(sim_data$ratio),
     main = "F")
hist(sim_data[sim_data$gender == "m", "ratio"],
     col = "darkblue", 
     breaks = 10,
     xlab = "length/width ratio",
     xlim = range(sim_data$ratio),
...
Hoe kleiner de _sample size_ hoe lastiger het is de onderliggende verdeling vast te stellen. Speel maar eens met dit blokje code, en verander de steekproefgrootte (`n=`) in de `rnorm()` functie en de `breaks=` in de `hist()` functie. 
Neem voor _n_ bijvoorbeeld achtereenvolgens 10, 100, 1000, 10000, 10000 en voor _breaks_ 5, 10, 50. Wanneer zie je  

```{r sampling-demo, fig.asp=.75, out.width='60%', fig.align='center'}
data <- rnorm(n = 100)
hist(x = data,
     breaks = 10,
     col = "gold")
...
boxplot(formula = hand_length ~ tech_ID,
    data = sim_data,
    xlab = "Technician",
    ylab = "Handlengte (mm)",
    col = c("darkgreen", "darkred"))
...
We kunnen de _lattice_ package gebruiken om op beide variabelen uit te splitsen:

```{r two-variation-sources-lattice, fig.asp=.75, out.width='90%', fig.align='center', message=FALSE, warning=FALSE}
library(lattice)
attach(sim_data)
bwplot(gender ~ hand_length | factor(tech_ID), 
   xlab="hand lengte (mm)", ylab="Gender",
   main=NULL,
   layout=(c(1,2)))
```

De expressie `gender ~ hand_length | factor(tech_ID)` kan gelezen worden als "plot gender als een functie van hand_length en splits uit op factor(tech_ID)". De functie `factor()` wordt gebruikt om de tech_ID om te zetten van numeric naar integer.

Als alternatief kan ook de `ggplot2` package gebruikt worden. Deze is onderdeel van de _tidyverse_ maar gebruikt een eigen _syntax_ om de verschillende lagen van de plot op te bouwen. Kijk in onderstaande chunk of je begrijpt wat er gebeurt.

```{r two-variation-sources-ggplot2, fig.asp=.75, out.width='90%', fig.align='center'}
library(ggplot2)
